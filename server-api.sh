...
MASTER_SUBNETS=("192.168.20.0/24" "192.168.21.0/24" "192.168.22.0/24")
...
do
	...
	THIRD_OCTET=$(echo "${TEMP_NETWORK}" | awk -F. '{print $3}')
        ...
        do
			...
			TEMP_NEW_BACKEND=$(echo "${DATA}" | jq -r --arg name "master-${THIRD_OCTET}-${ip}" --arg ip "${NETWORK}${ip}" '.address = $ip | .name = $name')
